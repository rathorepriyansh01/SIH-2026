...
def get_scan_history(db: Session = Depends(get_db)):
    """Fetch all previous scan records sorted by created_at descending."""
    ...
    return {"success": True, "count": len(result), "history": result}

@router.get("/history/{scan_id}", status_code=status.HTTP_200_OK)
def get_scan_detail(scan_id: str, db: Session = Depends(get_db)):
    """Fetch full details for a specific scan ID by joining with advisory details."""
    scan = db.query(ScanHistory).filter(ScanHistory.id == scan_id).first()
    if not scan:
        raise HTTPException(status_code=404, detail=f"Scan record with ID '{scan_id}' not found.")
        
    advisory = AdvisoryService.get_advisory(scan.disease)
    
    confidence_level = "High" if scan.confidence >= 80.0 else ("Moderate" if scan.confidence >= 60.0 else "Low")
    ...
    return {
        ...
        "scan_metadata": {
            "scan_id": scan.id,
            "timestamp": scan.created_at.isoformat() + "Z" if scan.created_at else None,
            "image_path": scan.image_path
        }
    }
